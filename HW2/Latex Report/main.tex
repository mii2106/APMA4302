...
First, lets assume that there exist two problems described by:
...
Also, it can be stablished that $\lVert \mathbf{A}\mathbf{u}\lVert = \lVert\mathbf{b}\lVert \le \lVert \mathbf{A}\lVert\lVert\mathbf{u}\lVert$, and therefore $\frac{\lVert\mathbf{b}\lVert}{\lVert\mathbf{u}\lVert} \le \lVert \mathbf{A}\lVert$

By multiplying both by $\lVert(\mathbf{u}-\hat{\mathbf{u}})\lVert$
...
Therefore , by using equation 6 and equation 7:
...
And as $\lVert \mathbf{e} \lVert = \lVert \mathbf{u}-\hat{\mathbf{u}} \lVert $ that means:
...
\section{Problem 2}
If $\mathbf{v}_j =\sin(j\pi x_i) $ is an eigenvector of $\mathbf{A}$, it should happen that $\mathbf{A}\mathbf{v}_j = \lambda_i \mathbf{v}_j$ 
...
Lets redefine as the arguments as sums of angles $\theta = j\pi i h $ and $\phi = j\pi h $
...
That $(\mathbf{A}\mathbf{v}_j)_i$ is the eigenvector of $\mathbf{A}$

\textbf{2)} The corresponding eigenvalue $\lambda$ is the value infront of the eigenvector.
...
For $j = 1, ... , m-1 $ (not including boundary conditions, where $\lambda$ should be zero)

\textbf{3)} If $m\rightarrow \infty$:
\begin{itemize}
    \item For j=1 is the lowest eigenvalue: $\frac{\pi}{2m}$ is small, then $\sin(\frac{\pi}{2m}) \approx \frac{\pi}{2m}$. Then

    \begin{equation}
        \lambda_{min} \approx 4m^2(\frac{\pi}{2m})^2 = \pi
    \end{equation}

    \item For j=m-1 is the lowest eigenvalue: $\frac{(m-1)\pi}{2m} = \frac{\pi}{2} - \frac{\pi}{2m}\approx \frac{\pi}{2}$ , then $\sin(\frac{\pi}{2}) = 1$. Then:

        \begin{equation}
        \lambda_{max} \approx 4m^2(1)^2 = 4m^2
    \end{equation}
\end{itemize}
...
For this problem, the code was created and its name is 'bvp.c'. Is added in the main file HW2. The file was made from tri.c code, and modfied by adding the term $gamma u$, thus the matrix $\mathbf{A}$ has the form:
